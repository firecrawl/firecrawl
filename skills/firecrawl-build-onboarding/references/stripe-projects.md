...
`--name firecrawl` makes the CLI write `FIRECRAWL_API_KEY` (plus `FIRECRAWL_API_BASE_URL` and `FIRECRAWL_DOCUMENTATION_URL`). Without it the key lands in `FIRECRAWL_API_API_KEY`, which the Firecrawl SDKs do not read.
