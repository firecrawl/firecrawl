...
The public [Running Locally](https://docs.firecrawl.dev/contributing/guide) guide is the canonical first-success path. It covers Node.js 22, pnpm `11.4.0`, Redis, the harness-managed PostgreSQL and RabbitMQ containers, and a verified local scrape. Note: building native dependencies during `pnpm install` requires the [FoundationDB client library](https://www.foundationdb.org/download/) (`fdb_c.h` and `libfdb_c`).
