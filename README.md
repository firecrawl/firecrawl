...
<p align="center">
  <a href="https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=firecrawl">
    <img src="./img/atlas-cloud-logo.png" alt="Atlas Cloud" width="200">
  </a>
</p>

> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=firecrawl)** is a full-modal, OpenAI-compatible AI inference platform. Firecrawl's LLM-dependent features (structured `json` extraction, `/extract`, `llms.txt` generation, image alt text) can be backed by Atlas as a drop-in provider — access DeepSeek, Qwen, GLM, Kimi, MiniMax and more through a single API, no multi-vendor setup needed. Budget-friendly: [coding plan](https://www.atlascloud.ai/console/coding-plan).

Point Firecrawl's `api` service at Atlas Cloud via environment variables (in `apps/api/.env`):
...
`deepseek-ai/deepseek-v4-pro` is a reasoning model — give LLM calls enough `max_tokens` (>= 512).
