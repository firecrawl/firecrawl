...
enum DenialReason {
  ...
  EXTERNAL_LINK = "This URL points to a different domain than the one being crawled. If external links are disabled, set allowExternalLinks: true in your crawl request to crawl them. If allowExternalLinks is already enabled, this URL was skipped because it is an external site's homepage (a URL with no path segments): Firecrawl follows external links to specific pages, but does not crawl external homepages, so a link or redirect landing on a bare domain root is not followed. allowSubdomains does not affect this, as it only covers subdomains of the site being crawled.",
