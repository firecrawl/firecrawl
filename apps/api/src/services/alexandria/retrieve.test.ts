...
it("forwards caller auth only for Bash source loading and never stores it", async () => {
  const credential = "Bearer test-private-credential";
  await run({ resultAuthorization: credential });
  expect(executions()[0][0].resultAuthorization).toBeUndefined();
  mocks.store.clear();
  mocks.request.mockClear();
  await run({
    resultAuthorization: credential,
    calls: [
      {
        provider: "firecrawl",
        capability: "bash",
        options: { requestId: "source-id", command: "ls" },
      },
    ],
  });
  expect(executions()[0][0].resultAuthorization).toBe(credential);
  expect(
    mocks.request.mock.calls
      .filter(([arg]) => arg.path !== "/v1/retrieve")
      .every(([arg]) => arg.resultAuthorization === undefined),
  ).toBe(true);
  expect([...mocks.store.values()].join("")).not.toContain(credential);
  mocks.store.clear();
  mocks.request.mockClear();
  await run({
    resultAuthorization: credential,
    calls: [
      {
        provider: "firecrawl",
        capability: "bash",
        options: { workspaceId: "existing", command: "ls" },
      },
    ],
  });
  expect(executions()[0][0].resultAuthorization).toBeUndefined();
