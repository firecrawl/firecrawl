import { describe, it, expect, beforeAll } from "@jest/globals";
import { agent, idmux, Identity } from "./lib";
import { describeIf, HAS_AI, TEST_PRODUCTION } from "../lib";

let identity: Identity;

beforeAll(async () => {
  identity = await idmux({
    name: "agent",
    concurrency: 100,
    tokens: 1000000,
  });
}, 10000);

describeIf(TEST_PRODUCTION || HAS_AI)("Agent", () => {
  it("successfully completes a basic agent extraction", async () => {
    const urls = ["https://en.wikipedia.org/wiki/Artificial_intelligence"];
    const prompt = "Extract information about artificial intelligence from the given URLs.";

    try {
      const result = await agent(
        {
          urls,
          prompt,
          maxCredits: 1000,
        },
        identity,
      );

      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.status).toBe("completed");
      expect(result.data).toBeDefined();
      expect(typeof result.data.finalAnalysis).toBe("string");
      expect(result.data.finalAnalysis.length).toBeGreaterThan(0);
      expect(Array.isArray(result.data.sources)).toBe(true);
      expect(result.data.sources.length).toBeGreaterThan(0);
      expect(Array.isArray(result.data.activities)).toBe(true);
      expect(result.data.activities.length).toBeGreaterThan(0);
    } catch (error) {
      console.error("Agent test failed with error:", error);
      throw error;
    }
  }, 300000); // 5 minutes timeout
});