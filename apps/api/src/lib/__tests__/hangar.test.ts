...
it("reads the recording timeline and makes HLS segment URLs absolute", async () => {
  vi.mocked(fetch).mockResolvedValue(
    new Response(
      "#EXTM3U\n#EXT-X-VERSION:3\n#EXTINF:5.250,\nsegments/0.ts\n#EXT-X-DISCONTINUITY\n#EXTINF:1.125,\nsegments/1.ts\n#EXT-X-ENDLIST\n",
    ),
  );
  const recording = await getHangarRecording(browser.playlist_url);
  expect(recording.durationMs).toBe(6375);
  expect(recording.playlist).toContain(
    "https://hangar.example/recordings/recording/segments/0.ts",
  );
  expect(recording.playlist).toContain("#EXT-X-DISCONTINUITY");
  expect(recording.playlist).toContain("#EXT-X-ENDLIST");
  expect(vi.mocked(fetch).mock.calls[0][1]).not.toHaveProperty(
    "headers.Authorization",
  );
});

it.each([401, 404, 409, 410])(
  "maps unavailable recording status %s to the existing replay 404",
  async status => {
    vi.mocked(fetch).mockResolvedValue(new Response("unavailable", { status }));
    await expect(
      getHangarRecording(browser.playlist_url),
...
it.each([
  ...
  "#EXTM3U\n#EXTINF:NaN,\nsegments/0.ts",
  "#EXTM3U\n#EXTINF:5,\n",
])("rejects malformed recording manifests", async playlist => {
  vi.mocked(fetch).mockResolvedValue(new Response(playlist));
  await expect(getHangarRecording(browser.playlist_url)).rejects.toMatchObject({
    status: 502,
  });
});
