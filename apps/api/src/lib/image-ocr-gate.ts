 ...
 * make up almost every crawl never pay for it. A lookup that fails or
 * finds no team record leaves image OCR off for that scrape: the team may
 * have opted out, and the deployment default must never override an
 * opt-out it could not read.
 */
...
async function resolveImageOcrEnabled(
  ...
  try {
    ...
    if (!acuc) {
      // No record for the team: its opt-out could not be read either, so
      // this is the same fail-closed case as a thrown lookup error.
      logger.warn("No team record found for image OCR; leaving it off", {
        teamId,
      });
      return false;
    }
