...
func TestToolDiscoveryAndExecution(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["exchange"].([]interface{})[0].(map[string]interface{})["provider"] == "firecrawl-contextual-discovery" {
			w.Write([]byte(`{"success":true,"data":{"exchange":[{"error":{"code":"invalid_options","message":"Invalid lookup"}}],"creditsCost":0}}`))
			return
		}
		...
		w.Write([]byte(`{"success":true,"scrape_id":"scrape-1","data":{"exchange":[{"provider":"p","capability":"a","creditsCost":2,"data":{"nested":[1,2]}},{"provider":"p","capability":"b","error":{"code":"unavailable","message":"unavailable"}}],"creditsCost":2}}`))
	}))
	defer server.Close()
	...
	result, err := client.ScrapeExchange(context.Background(), []ExchangeCall{{Provider: "p", Capability: "a"}}, &ExchangeOptions{RequestID: "retry-1"})
	if err != nil {
		t.Fatal(err)
	}
	if result.RequestID != "retry-1" || result.CreditsCost != 2 || !result.Exchange[1].Failed() {
	...
	var executionErr *ExchangeExecutionError
