...
app.post('/scrape', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userAgentHeader = headers?.['user-agent'] || headers?.['User-Agent'] || headers?.['USER-AGENT'];
    const customUserAgent = userAgentHeader || undefined;
    
    // Remove user-agent from headers as it's set at context level
