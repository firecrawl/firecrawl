...
class TestAsyncHttpClient:
    """Test cases for AsyncHttpClient security fixes."""
    
    @pytest.fixture
    def client(self):
        """Create a test client with API key."""
        return AsyncHttpClient("test-api-key", "https://api.firecrawl.dev")
    
    @pytest.fixture
    def client_no_key(self):
        """Create a test client without API key."""
        return AsyncHttpClient(None, "https://api.firecrawl.dev")
    
    @pytest.fixture
    def client_empty_key(self):
        """Create a test client with empty API key."""
        return AsyncHttpClient("   ", "https://api.firecrawl.dev")
    ...
    def test_headers_with_api_key_same_host(self, client):
        """Test that Authorization header is included for same host."""
        headers = client._headers("/v2/scrape")
        assert "Authorization" in headers
        assert headers["Authorization"] == "Bearer test-api-key"

    def test_headers_with_api_key_different_host(self, client):
    ...
    def test_headers_with_empty_api_key(self, client_empty_key):
        """Test that no Authorization header is added for empty/whitespace API key."""
        headers = client_empty_key._headers("/v2/scrape")
        assert "Authorization" not in headers

    def test_headers_with_idempotency_key(self, client):
        """Test that idempotency key is always added regardless of host."""
        headers = client._headers("/v2/scrape", idempotency_key="test-key")
        assert "Authorization" in headers
        assert "x-idempotency-key" in headers
        assert headers["x-idempotency-key"] == "test-key"

    def test_headers_idempotency_key_different_host(self, client):
    ...
    @pytest.mark.asyncio
    async def test_post_same_host_includes_auth(self, client):
    ...
    @pytest.mark.asyncio
    async def test_post_different_host_excludes_auth(self, client):
        """Test that POST to different host excludes Authorization header."""
        with patch.object(client._client, 'post', new_callable=AsyncMock) as mock_post:
            await client.post("https://malicious.com/v2/scrape", {"url": "https://example.com"})
    ...
    @pytest.mark.asyncio
    async def test_get_same_host_includes_auth(self, client):
        """Test that GET to same host includes Authorization header."""
        with patch.object(client._client, 'get', new_callable=AsyncMock) as mock_get:
            await client.get("/v2/status")
    ...
    @pytest.mark.asyncio
    async def test_get_different_host_excludes_auth(self, client):
        """Test that GET to different host excludes Authorization header."""
        with patch.object(client._client, 'get', new_callable=AsyncMock) as mock_get:
            await client.get("https://malicious.com/v2/status")
    ...
    @pytest.mark.asyncio
    async def test_delete_same_host_includes_auth(self, client):
        ...
        with patch.object(client._client, 'delete', new_callable=AsyncMock) as mock_delete:
            await client.delete("/v2/jobs/123")
    ...
    @pytest.mark.asyncio
    async def test_delete_different_host_excludes_auth(self, client):
        """Test that DELETE to different host excludes Authorization header."""
        with patch.object(client._client, 'delete', new_callable=AsyncMock) as mock_delete:
            await client.delete("https://malicious.com/v2/jobs/123")
